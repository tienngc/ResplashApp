...
class UnsplashDio extends AppDio {
  ...
  UnsplashDio({required Connectivity connectivity}) : _connectivity = connectivity;
  ...
  Dio _get() {
    return Dio()
      ..options = BaseOptions(
        ...
        headers: {'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': 'Client-ID ${EnvConfig.unsplashApiKey}'},
        connectTimeout: Duration(milliseconds: _connectTimeout),
        receiveTimeout: Duration(milliseconds: _receiveTimeout),
      )
      ..interceptors.addAll([LoggingInterceptor(), ConnectivityInterceptor(connectivity: _connectivity)]);
  }
}
